...
function publicState(room) {
  return { code: room.code, hostId: room.hostId, categoryKey: room.categoryKey,
...
io.on('connection', socket => {
  socket.on('room:create', ({ code, name }, callback) => {
    ...
    const room = { code, hostId: socket.id, categoryKey: null, usedQuestions: new Set(), activeQuestion: null, timer: null, players: [{ id: socket.id, name, score: 0 }], spectators: [], turnIndex: 0 };
  ...
  socket.on('category:select', ({ categoryKey }, callback) => {
    ...
    if (room.categoryKey && room.categoryKey !== categoryKey) return callback?.({ error: 'La categoría ya fue elegida para esta partida.' });
    room.categoryKey = categoryKey; emitState(room); callback?.({ ok: true });
  });
  socket.on('question:select', ({ subKey, qIndex }, callback) => {
    ...
    if (!room || room.activeQuestion || socket.id !== turnId || !question || room.usedQuestions.has(key)) return callback?.({ error: 'No podés seleccionar esta pregunta.' });
    room.activeQuestion = { categoryKey: room.categoryKey, subKey, qIndex, endsAt: Date.now() + 60000 };
    room.timer = setTimeout(() => endQuestion(room, 'Tiempo agotado'), 60000);
    io.to(room.code).emit('question:opened', { ...room.activeQuestion, question: { points: question.points, q: question.q, options: question.options } });
  ...
  socket.on('answer:submit', ({ answer }, callback) => {
    ...
    const question = trivia[active.categoryKey].subcategories[active.subKey].questions[active.qIndex]; const correct = Number(answer) === question.correct;
    const player = room.players.find(item => item.id === socket.id); player.score += correct ? question.points : -Math.floor(question.points / 2);
    io.to(room.code).emit('answer:result', { answer: Number(answer), correctIndex: question.correct, correct, playerName: player.name });
    endQuestion(room, correct ? 'Respuesta correcta' : 'Respuesta incorrecta'); callback?.({ ok: true });
  });
